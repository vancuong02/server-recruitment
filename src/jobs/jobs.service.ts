...
export class JobsService {
    ...
    async findAll(query: QueryJobDto) {
        const { current = 1, pageSize = 10, skills, locations, levels, typeWorks, typeContracts } = query
        ...
        if (typeContracts) {
            condition.typeContracts = {
                $in: typeContracts.split(',').map((typeContract) => typeContract.trim()),
            }
        }

        const [items, total] = await Promise.all([
            this.jobModel.find(condition).populate('companyId', '_id name logo').skip(skip).limit(itemsPerPage).lean(),
    ...
    async findOne(id: string) {
        ...
        return this.jobModel.findById(id).populate('companyId', '_id name logo').lean()
    }

    async findAllByCompany(companyId: string) {
        return this.jobModel.find({ companyId }).populate('companyId', '_id name logo').lean()
    }
